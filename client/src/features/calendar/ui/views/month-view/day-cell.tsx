...
import { transition } from "@/features/calendar/config/animations"
import { EventListDialog } from "@/features/calendar/ui/dialogs/events-list-dialog"
import { DroppableArea } from "@/features/calendar/ui/dnd/droppable-area"
import { getMonthCellEvents } from "@/features/calendar/model/helpers"
import { useMediaQuery } from "@/features/calendar/model/hooks"
import type {
  ICalendarCell,
  IEvent,
} from "@/features/calendar/model/interfaces"
import { EventBullet } from "@/features/calendar/ui/views/month-view/event-bullet"
import { MonthEventBadge } from "@/features/calendar/ui/views/month-view/month-event-badge"
...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const cellContent = useMemo(
    () => (
      <motion.div
        className={cn(
          "flex h-full flex-col gap-1 border-t border-l lg:min-h-40",
      ...
      >
        <DroppableArea date={date} className="h-full w-full py-2">
          ...
          {showDesktopMore && (
            <motion.div
              className={cn(
                "my-2 h-4.5 px-1.5 text-end text-xs font-semibold text-muted-foreground",
                !currentMonth && "opacity-50"
              )}
